...
class Teste {
    public static main(): Cliente[] {
        ...
        console.log("--- Array de Clientes criado");
        return clientes;
    }

    
    public static imprimirDados(clientes: Cliente[]): void {
        console.log("\n--- Impressão de Clientes, Animais e Procedimentos");
...
class User implements IAuthenticable, IMailable {
    private email: string;
    private name: string;
    private login: string;
    private password: string;

    constructor(email: string, name: string, login: string, password: string) {
        this.email = email;
        this.name = name;
        this.login = login;
...
class Admin implements IAuthenticable, IMailable {
    private email: string;
    private name: string;
    private login: string;
    private password: string;
    ...
    constructor(email: string, name: string, login: string, password: string, role: string) {
        this.email = email;
        this.name = name;
        this.login = login;
        this.password = password;
...
class Mail {
    public static sendMail(user: IMailable): void {
        console.log(`[OK] Email enviado para: ${user.getEmail()}`);
    }
}

let u1: User = new User("joao@gmail.com", "Joao Silva", "joao", "joao");
let a1: Admin = new Admin("maria@gmail.com", "Maria Souza", "maria", "maria", "Gerente");
let a2: Admin = new Admin("marlon@gmail.com", "Marlon Santos", "marlon", "marlon123", "Administrador");
let ret: boolean;

let users: (IAuthenticable & IMailable)[] = [u1, a1, a2];

for (let u of users) {
    ret = AuthenticateUsers.authenticate(
        u,
        (login) => console.log(`[OK] Usuario autenticado: ${login}`),
        (login) => console.error(`[ERRO] Erro de autenticacao: ${login}`)
    );
